...
resource "azurerm_network_interface_security_group_association" "mySecGroupAssociationMasters" {
    count                     = length(var.masters)
    network_interface_id      = azurerm_network_interface.myNicMasters[count.index].id
